...
void twoMisThreePartSearch(char *pattern, Idx2BWT *idx2BWT, int firstPart, int secondPart, long *sumVisitedVertices, long *saCountSum) {
    ...
    for (i = patternLength - 2; i >= firstPart + secondPart; i--) 
    { 
        visitedVertices++;
    ...
    if (l <= r) 
    {
        for (i = firstPart + secondPart - 1; i >= 0; i--) 
        {
            ...
            for (firstSymbol = 0; firstSymbol < ALPHABET_SIZE; firstSymbol++) 
            {
                ...
                for(temp = i - 1; temp >= firstPart; temp--) 
                {   
                    visitedVertices++;
                    if (l > r)
                        break;
                ...
                int jStartFrom = i - 1;
                if (firstPart - 1 < jStartFrom) {
                    jStartFrom = firstPart - 1;
                }
    ...
    if (l <= r) 
    {
        for (i = firstPart - 1; i >= 0; i--) 
        {
            ...
            for (firstSymbol = 0; firstSymbol < ALPHABET_SIZE; firstSymbol++) 
            {
                ...
                {   
                    visitedVertices++;
                    BWTSARangeBackward_Bidirection(idx2BWT,pattern[j],&l,&r, &rev_l, &rev_r); 
                }           
                for(j = firstPart + secondPart; j < patternLength; j++) 
                {
                    visitedVertices += ALPHABET_SIZE;
                    BWTAllSARangesForward_Bidirection(idx2BWT,l,r, rev_l, rev_r, all_l_second_m,all_r_second_m, all_rev_l_second_m,all_rev_r_second_m);
                    //fprintf(f, "    j = %d: ", j);
